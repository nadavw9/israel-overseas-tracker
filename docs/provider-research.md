...
| --- | --- | --- | --- |
| API-Football | Football player profiles and season statistics; leagues, fixtures, events, lineups | $0 plan, 100 requests/day, player statistics listed | **Primary football path.** One request per verified player-season binding; nightly refresh. |
| API-NBA | NBA players, games, events, standings, statistics | Official API-Sports page lists a free plan with 100 requests/day | **Next basketball candidate.** Verify the exact player-stat endpoint and competition coverage before binding. |
