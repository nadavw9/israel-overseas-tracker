...
function newest<T extends { id: string }>(records: readonly T[], timestamp: keyof T, description: string, athleteId: string): T {
  const selected = [...records].sort((left, right) => {
    const compare = String(right[timestamp]).localeCompare(String(left[timestamp]))
    return compare || left.id.localeCompare(right.id)
  })[0]
...
export function compileRegistryBundle(input: unknown, asOfDate: string): RegistryAthlete[] {
  const bundle = createRegistryBundleSchema(asOfDate).parse(input)
  return bundle.athletes.filter((athlete) => athlete.visibility === 'public').map((athlete) => {
    ...
    const eligibility = newest(bundle.evidence.filter((claim): claim is VerifiedEligibility => claim.athleteId === athlete.id && isVerifiedEligibility(claim)), 'retrievedAt', 'verified eligibility claim', athlete.id)
    const binding = newest(bundle.providerBindings.filter((record): record is VerifiedBinding => record.athleteId === athlete.id && isVerifiedBinding(record) && record.competition === affiliation.competition), 'verifiedAt', 'verified provider binding matching affiliation competition', athlete.id)
    const approved = bundle.media.filter((record): record is ApprovedMedia => record.athleteId === athlete.id && isApprovedMedia(record))
...
export function compilePublicRegistry(asOfDate: string): RegistryAthlete[] {
  return compileRegistryBundle(bundledData, asOfDate)
}

export const publicRegistry = compilePublicRegistry(new Date().toISOString().slice(0, 10))
