...
The current census and participation registry is manual. ESPN NBA and public ESPN soccer roster performance collection is scheduled, not live play-by-play. Any future feed must preserve observed time, provider identity, competition, and season.
...
The nightly schedule is a bounded end-of-day polling SLA, not a claim of second-by-second live coverage. Live or near-live behavior is only possible when a licensed provider and competition-specific entitlement exist. The current public snapshot has 37 verified athletes, eleven adapter-bound records, and eleven numeric performance records (three NBA plus eight football); the remaining 26 are still published as identity records with explicit `not-integrated` performance status. A hosting deployment must publish the generated artifact after the workflow succeeds; the repository workflow intentionally does not commit or deploy data on its own.
