...
| MLB Stats API | Current MLB player identity, team, and pitching/batting season totals; no key observed | Dean Kremer (MLB person 665152) responded with a current-team record and 2026 pitching totals; official MLB terms prohibit automated scripts interacting with MLB digital properties | **Do not automate or publish yet.** Keep the lead in `data/review/mlb-coverage-audit.json` and request written permission or use a licensed MLB data partner before binding. |
